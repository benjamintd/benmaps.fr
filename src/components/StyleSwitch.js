...
class StyleSwitch extends Component {
  render() {
    return (
      <div
        ...
        onClick={() => {
          let newStyle;
          if (this.props.mapStyle.indexOf("streets") > -1)
            newStyle = "satellite";
          else if (this.props.mapStyle.indexOf("satellite") > -1)
            newStyle = "streets";

          if (this.props.mapStyle.indexOf("traffic") > -1)
            newStyle += "-traffic";
      ...
      >
        <img
          className="cover relative top w-full h-full"
          alt="switch style"
          src={this.imgUrl}
          key={this.imgUrl}
        />
  ...
  get imgUrl() {
    var style =
      this.props.mapStyle.indexOf("streets") > -1
        ? "mapbox/satellite-v9/"
        : "benjamintd/cj0szkyh5009i2slfhsmxhtni/";
    var base = "https://api.mapbox.com/styles/v1/" + style + "static/";
    var coords =
      this.props.center.join(",") + "," + Math.max(0, this.props.zoom - 4);
    var size = "56x100@2x";
