...
class ContextMenu extends Component {
  render() {
    ...
    return (
      <div
        className="absolute bg-white w240 hmax300 shadow-darken25 border--gray"
        style={style}
      >
        ...
        <div
          onClick={() => this.setDirections("directionsFrom")}
          className="px12 py6 bg-darken10-on-hover cursor-pointer"
        >
          directions from this place
        </div>
        <div
          onClick={() => this.setDirections("directionsTo")}
          className="px12 py6 bg-darken10-on-hover cursor-pointer"
        >
  ...
  formatCoordinates() {
    return this.props.coordinates.map(e => e.toFixed(6)).join(",");
  }
